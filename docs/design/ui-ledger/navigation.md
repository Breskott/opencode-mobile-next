...
- `home-shell`: {server name} · Workspace / Files / Activity / More (screen, `lib/ui/screens/home_screen.dart`); gates: named route '/home' is ungated; via '/' requires conn.profile != null && conn.hasConnectedServer
- `workspace`: Workspace (tab, `lib/ui/screens/workspace_screen.dart`)
- `files`: Files (tab, `lib/ui/screens/files_screen.dart`); gates: conn.capabilities.fileBrowsing; add-to-prompt / attach affordances only when opened from a chat (widget.handoff / widget.onAttachFile != null)
- `activity`: Activity (screen, `lib/ui/screens/activity_screen.dart`)
- `more`: More (tab, `lib/ui/screens/library_screen.dart`)
...
graph LR
  about["about"]:::ext
  about_settings["about-settings"]:::ext
  ...
  sessions_tab_delete_sheet(["sessions-tab-delete-sheet"])
  settings["settings"]:::ext
  ...
  command_launcher_sheet --> tools
  command_launcher_sheet --> references
  command_launcher_sheet --> settings
  command_launcher_sheet --> app_diagnostics
  command_launcher_sheet --> appearance_picker_sheet
  ...
  diff_sheet --> diff_view
  sessions_tab --> chat
  sessions_tab --> sessions_tab_delete_sheet
  ...
  sessions_tab -.-> embedded_context_menu_region
...
graph LR
  ...
  catalog["catalog"]
  catalog_model_details_sheet(["catalog-model-details-sheet"])
  chat["chat"]:::ext
  coding_settings["coding-settings"]
  ...
  settings --> about_settings
  settings --> plugins_settings
  settings --> settings_disconnect_sheet
  settings_disconnect_sheet --> servers
  coding_settings --> coding_settings_shell_sheet
  coding_settings --> model_picker_sheet
  ...
  team_plugin_sheet --> team_host_sheet
  team_plugin_sheet --> team_turn_off_sheet
  plugins --> plugins_clear_mappings_sheet
  ...
  catalog --> catalog_model_details_sheet
  ...
  coding_settings -.-> embedded_product_states
...
graph LR
  ...
  activity["activity"]:::ext
  chat["chat"]:::ext
  coding_settings["coding-settings"]:::ext
  commands["commands"]:::ext
...
| `shortcuts-help-dialog` | dialog | 1 / 3 | `global-shortcuts` / global-shortcuts-help<br>`command-palette-dialog` / command-palette-dialog-cmd-shortcuts<br>`more` / more-keyboard-shortcuts-tile | _none_ |
...
| `todos-sheet` | sheet | 3 / 4 | `session-menu-sheet` / session-menu-sheet-todos | _none_ |
| `diff-sheet` | sheet | unreachable | _none_ | diff-sheet-file-row -> `diff-view` |
| `sessions-tab` | tab | unreachable | _none_ | sessions-tab-start-one -> `chat`<br>sessions-tab-row -> `chat`<br>sessions-tab-row-swipe-delete -> `sessions-tab-delete-sheet`<br>sessions-tab-row-menu-rename -> `sessions-tab-rename-dialog`<br>sessions-tab-row-menu-delete -> `sessions-tab-delete-sheet`<br>sessions-tab-row-context-open -> `chat`<br>sessions-tab-row-context-rename -> `sessions-tab-rename-dialog`<br>sessions-tab-row-context-delete -> `sessions-tab-delete-sheet`<br>sessions-tab-new-chat-fab -> `chat`<br>(embedded) -> `embedded-context-menu-region` |
| `sessions-tab-rename-dialog` | dialog | unreachable | `sessions-tab` / sessions-tab-row-menu-rename<br>`sessions-tab` / sessions-tab-row-context-rename | _none_ |
| `sessions-tab-delete-sheet` | sheet | unreachable | `sessions-tab` / sessions-tab-row-swipe-delete<br>`sessions-tab` / sessions-tab-row-menu-delete<br>`sessions-tab` / sessions-tab-row-context-delete | _none_ |
...
| `session-import` | screen | 1 / 3 | `more` / more-import-session-tile | session-import-change-destination -> `session-import-destination-sheet`<br>session-import-open -> `chat` |
...
| `voice-notices` | screen | 3 / 5 | `appearance-settings`<br>`about-settings` / about-settings-voice-notices | (embedded) -> `embedded-product-states` |
| `voice-model-setup-sheet` | sheet | 2 / 3 | `voice-composer-sheet` / voice-composer-sheet-model-settings<br>`chat` | voice-model-setup-sheet-pack-delete -> `voice-model-setup-sheet-delete-dialog` |
| `voice-model-setup-sheet-delete-dialog` | dialog | 3 / 4 | `voice-model-setup-sheet` / voice-model-setup-sheet-pack-delete | _none_ |
...
| `project-health` | screen | 2 / 3 | `manage-project` / manage-project-health<br>`chat`<br>`command-launcher-sheet` / chat-command-health | project-health-git-init -> `project-health-git-init-dialog`<br>project-health-git-init-retry -> `project-health-git-init-dialog`<br>(embedded) -> `embedded-product-states` |
| `project-health-git-init-dialog` | dialog | 3 / 4 | `project-health` / project-health-git-init<br>`project-health` / project-health-git-init-retry | _none_ |
...
| `worktrees` | screen | 3 / 5 | `manage-project` / manage-project-worktrees | worktrees-create -> `worktrees-create-dialog`<br>worktrees-glossary-label -> `info-label-sheet`<br>worktrees-tile-menu-reset -> `worktrees-reset-dialog`<br>worktrees-tile-menu-remove -> `worktrees-remove-dialog`<br>worktrees-tile-context-reset -> `worktrees-reset-dialog`<br>worktrees-tile-context-remove -> `worktrees-remove-dialog`<br>(embedded) -> `embedded-context-menu-region`<br>(embedded) -> `embedded-product-states`<br>(embedded) -> `embedded-info-label` |
| `worktrees-reset-dialog` | dialog | 4 / 6 | `worktrees` / worktrees-tile-menu-reset<br>`worktrees` / worktrees-tile-context-reset | _none_ |
| `worktrees-create-dialog` | dialog | 4 / 6 | `worktrees` / worktrees-create | _none_ |
| `worktrees-remove-dialog` | dialog | 4 / 6 | `worktrees` / worktrees-tile-menu-remove<br>`worktrees` / worktrees-tile-context-remove | _none_ |
| `embedded-mobile-task-list` | overlay | 1 / 2 | `embedded-tool-card` / (embedded) | _none_ |
| `embedded-running-agents-strip` | overlay | unreachable | _none_ | _none_ |
...
| Page | Kind | Depth | Inbound (page / element) | Outbound (element -> page) |
|---|---|---|---|---|
| `files` | tab | 0 / 2 | `home-shell` / home-shell-tab-files<br>`chat`<br>`command-launcher-sheet` / chat-command-files | files-changes-card -> `files-changes-sheet`<br>files-row -> `files-file-viewer-sheet`<br>files-row-long-press -> `files-row-actions-sheet`<br>files-row-menu-open -> `files-file-viewer-sheet`<br>files-row-menu-review -> `review-workspace`<br>files-symbol-row -> `files-file-viewer-sheet`<br>files-row -> `review-workspace`<br>(embedded) -> `embedded-context-menu-region`<br>(embedded) -> `embedded-product-states` |
| `files-row-actions-sheet` | sheet | 1 / 3 | `files` / files-row-long-press | files-row-actions-sheet-open -> `files-file-viewer-sheet`<br>files-row-actions-sheet-review -> `review-workspace` |
| `files-changes-sheet` | sheet | 1 / 3 | `files` / files-changes-card | files-changes-sheet-review-all -> `review-workspace`<br>files-changes-sheet-file -> `review-workspace` |
| `files-file-viewer-sheet` | sheet | 1 / 3 | `files` / files-row<br>`files` / files-symbol-row<br>`files` / files-row-menu-open<br>`files-row-actions-sheet` / files-row-actions-sheet-open | (embedded) -> `embedded-file-preview-body` |
| `review-workspace` | screen | 1 / 3 | `files-changes-sheet` / files-changes-sheet-review-all<br>`files-changes-sheet` / files-changes-sheet-file<br>`files` / files-row-menu-review<br>`files-row-actions-sheet` / files-row-actions-sheet-review<br>`files` / files-row<br>`chat`<br>`command-launcher-sheet` / chat-command-diff<br>`session-menu-sheet` / session-menu-sheet-changes | review-workspace-ask-file -> `review-comment-sheet`<br>review-workspace-file-actions-ask -> `review-comment-sheet`<br>review-workspace-selection-comment -> `review-comment-sheet`<br>(embedded) -> `embedded-product-states` |
...
| `diff-view` | screen | 2 / 3 | `staged-revert` / staged-revert-file-row<br>`chat` / chat-appbar-demo-review-changes<br>`chat` / chat-body-demo-review-changes<br>`permission-sheet` / permission-sheet-see-full-diff<br>`diff-sheet` / diff-sheet-file-row | _none_ |
...
| `embedded-file-preview-body` | overlay | 1 / 2 | `file-preview-sheet` / (embedded)<br>`files-file-viewer-sheet` / (embedded)<br>`skill-activation-sheet` / (embedded)<br>`skills` / (embedded) | (embedded) -> `embedded-markdown-text` |
| `run-command-dialog` | dialog | 2 / 4 | `plugins` / plugins-review-command<br>`commands` / commands-row | (embedded) -> `embedded-session-inventory-footer` |
...
| `external-agents` | screen | 2 / 1 | `servers` / servers-external-agents<br>`servers-welcome` / servers-welcome-external-agents | external-agents-add -> `add-agent`<br>external-agents-row -> `external-agent-detail`<br>external-agents-retry-delete -> `external-agents-delete-sheet`<br>-> `external-link-dialog` |
...
| `host-management` | screen | 3 / 5 | `server-settings` / server-settings-host-management | _none_ |
...
| `server-settings` | screen | 2 / 4 | `settings` / settings-category-server | server-settings-manage-profiles -> `servers`<br>server-settings-host-management -> `host-management`<br>server-settings-updates-managed -> `termux-setup`<br>server-settings-updates-restart -> `server-settings-restart-dialog`<br>server-settings-updates-upgrade -> `server-settings-upgrade-sheet`<br>(embedded) -> `embedded-product-states` |
| `server-settings-restart-dialog` | dialog | 3 / 5 | `server-settings` / server-settings-updates-restart | _none_ |
| `server-settings-upgrade-sheet` | sheet | 3 / 5 | `server-settings` / server-settings-updates-upgrade | _none_ |
| `tailscale-setup` | screen | 2 / 1 | `servers` / servers-tailscale<br>`servers-welcome` / servers-welcome-tailscale<br>`profile-editor` / profile-editor-tailscale-help | tailscale-setup-install -> `external-link-dialog`<br>tailscale-setup-continue -> `profile-editor`<br>tailscale-setup-serve-docs -> `external-link-dialog`<br>tailscale-setup-android-docs -> `external-link-dialog` |
| `usage` | screen | 2 / 4 | `settings` / settings-category-usage | usage-budget-usd -> `usage-budget-dialog`<br>usage-budget-tokens -> `usage-budget-dialog`<br>usage-budget-clear-all -> `usage-budget-clear-dialog` |
...
| `embedded-team-discovery-card` | overlay | 2 / 4 | `plugins-settings`<br>`plugins-settings` / (embedded) | _none_ |
...
| `team-turn-off-sheet` | sheet | 4 / 6 | `team-plugin-sheet` / team-plugin-sheet-turn-off | _none_ |
...
| `embedded-team-phone-section` | overlay | 3 / 5 | `team-plugin-sheet`<br>`team-plugin-sheet` / (embedded) | embedded-team-phone-section-stop -> `team-phone-stop-sheet`<br>embedded-team-phone-section-open-setup -> `termux-setup`<br>embedded-team-phone-section-keep-running -> `team-phone-tips-sheet`<br>embedded-team-phone-section-remove -> `team-phone-remove-sheet` |
| `team-phone-stop-sheet` | sheet | 4 / 6 | `embedded-team-phone-section` / embedded-team-phone-section-stop | _none_ |
| `team-phone-remove-sheet` | sheet | 4 / 6 | `embedded-team-phone-section` / embedded-team-phone-section-remove | _none_ |
| `team-phone-tips-sheet` | sheet | 4 / 6 | `embedded-team-phone-section` / embedded-team-phone-section-keep-running | _none_ |
| `embedded-team-phone-reoffer-card` | overlay | 2 / 4 | `plugins-settings`<br>`plugins-settings` / (embedded) | embedded-team-phone-reoffer-card-set-up -> `termux-setup` |
...
| Page | Kind | Depth | Inbound (page / element) | Outbound (element -> page) |
|---|---|---|---|---|
| `about` | screen | 2 / 1 | `about-settings` / about-settings-privacy<br>`about-settings` / about-settings-app-details<br>`system` / system-named-route-about-lib-main-dart-1238-to-about<br>`servers` / servers-about<br>`servers-welcome` / servers-welcome-about | about-tab-privacy -> `about-privacy-tab`<br>about-tab-open-source -> `about-open-source-tab`<br>(embedded) -> `embedded-markdown-text` |
| `about-open-source-tab` | tab | 3 / 2 | `about` / about-tab-open-source | _none_ |
| `about-privacy-tab` | tab | 3 / 2 | `about` / about-tab-privacy | _none_ |
| `app-diagnostics` | screen | 2 / 3 | `diagnostics-settings` / diagnostics-settings-app-diagnostics<br>`chat`<br>`command-palette-dialog` / command-palette-dialog-cmd-diagnostics<br>`system` / system-named-route-debug-via-desktop-command-diagnostics-lib-main-d-to-app-diagnostics<br>`command-launcher-sheet` / chat-command-debug | app-diagnostics-clear -> `app-diagnostics-clear-sheet` |
| `app-diagnostics-clear-sheet` | sheet | 3 / 4 | `app-diagnostics` / app-diagnostics-clear | _none_ |
...
| `catalog` | screen | 1 / 3 | `more` / more-models-agents-tile | catalog-legacy-model-row -> `catalog-model-details-sheet`<br>catalog-legacy-provider-row -> `integrations` |
| `catalog-model-details-sheet` | sheet | 2 / 4 | `catalog` / catalog-legacy-model-row | _none_ |
...
| `more` | tab | 0 / 2 | `home-shell` / home-shell-tab-more<br>`system` / system-desktop-command-shortcut-mod-4-go-3-lib-main-dart-1126-to-more | more-termux-setup-tile -> `termux-setup`<br>more-settings-tile -> `settings`<br>more-models-agents-tile -> `catalog`<br>more-terminal-tile -> `terminal`<br>more-providers-tile -> `integrations`<br>more-mcp-tile -> `integrations`<br>more-commands-tools-tile -> `capabilities`<br>more-plugins-tile -> `plugins`<br>more-import-session-tile -> `session-import`<br>more-setup-guide-tile -> `guide`<br>more-keyboard-shortcuts-tile -> `shortcuts-help-dialog` |
...
| `plugins` | screen | 1 / 3 | `more` / more-plugins-tile | plugins-clear-mappings -> `plugins-clear-mappings-sheet`<br>plugins-review-command -> `run-command-dialog`<br>plugins-manage-mapping -> `plugins-mapping-dialog`<br>-> `chat` |
| `plugins-clear-mappings-sheet` | sheet | 2 / 4 | `plugins` / plugins-clear-mappings | _none_ |
| `plugins-mapping-dialog` | dialog | 2 / 4 | `plugins` / plugins-manage-mapping | _none_ |
| `saved-permissions` | screen | 3 / 5 | `privacy-settings` / privacy-settings-saved-permissions | saved-permissions-revoke -> `saved-permissions-revoke-dialog`<br>(embedded) -> `embedded-product-states` |
...
| `coding-settings` | screen | 2 / 4 | `settings` / settings-category-coding | coding-settings-default-shell -> `coding-settings-shell-sheet`<br>coding-settings-default-model -> `model-picker-sheet`<br>coding-settings-default-agent -> `model-picker-sheet`<br>(embedded) -> `embedded-product-states` |
| `coding-settings-shell-sheet` | sheet | 3 / 5 | `coding-settings` / coding-settings-default-shell | _none_ |
| `appearance-settings` | screen | 2 / 4 | `settings` / settings-category-appearance | appearance-settings-mode -> `appearance-picker-sheet`<br>appearance-settings-language -> `language-sheet`<br>appearance-settings-theme-pack -> `theme-pack-preview-sheet`<br>-> `voice-notices` |
| `privacy-settings` | screen | 2 / 4 | `settings` / settings-category-privacy | privacy-settings-saved-permissions -> `saved-permissions`<br>privacy-settings-clear-queued -> `privacy-settings-clear-queued-sheet`<br>privacy-settings-clear-drafts -> `privacy-settings-clear-drafts-sheet` |
| `privacy-settings-clear-drafts-sheet` | sheet | 3 / 5 | `privacy-settings` / privacy-settings-clear-drafts | _none_ |
| `privacy-settings-clear-queued-sheet` | sheet | 3 / 5 | `privacy-settings` / privacy-settings-clear-queued | _none_ |
| `diagnostics-settings` | screen | 2 / 4 | `settings` / settings-category-diagnostics | diagnostics-settings-app-diagnostics -> `app-diagnostics` |
| `about-settings` | screen | 2 / 4 | `settings` / settings-category-about | about-settings-setup-guide -> `guide`<br>about-settings-privacy -> `about`<br>about-settings-voice-notices -> `voice-notices`<br>about-settings-app-details -> `about` |
| `plugins-settings` | screen | 2 / 4 | `settings` / settings-category-plugins | plugins-settings-ai-team-row -> `team-plugin-sheet`<br>-> `embedded-team-phone-reoffer-card`<br>(embedded) -> `embedded-team-phone-reoffer-card`<br>-> `embedded-team-discovery-card`<br>(embedded) -> `embedded-team-discovery-card` |
| `team-plugin-sheet` | sheet | 3 / 5 | `plugins-settings` / plugins-settings-ai-team-row | team-plugin-sheet-how -> `team-host-guide-sheet`<br>team-plugin-sheet-add-manually -> `team-host-sheet`<br>team-plugin-sheet-turn-off -> `team-turn-off-sheet`<br>-> `embedded-team-phone-section`<br>(embedded) -> `embedded-team-phone-section`<br>(embedded) -> `embedded-team-technical-value` |
| `settings` | screen | 1 / 3 | `more` / more-settings-tile<br>`chat`<br>`global-shortcuts` / global-shortcuts-settings<br>`command-palette-dialog` / command-palette-dialog-cmd-settings<br>`system` / system-desktop-shortcut-mod-command-palette-settings-opensettings-l-to-settings<br>`command-launcher-sheet` / chat-command-status | settings-category-server -> `server-settings`<br>settings-category-coding -> `coding-settings`<br>settings-category-background -> `background-settings`<br>settings-category-appearance -> `appearance-settings`<br>settings-category-privacy -> `privacy-settings`<br>settings-category-usage -> `usage`<br>settings-category-quota -> `provider-quota`<br>settings-category-diagnostics -> `diagnostics-settings`<br>settings-category-about -> `about-settings`<br>settings-category-plugins -> `plugins-settings`<br>settings-disconnect -> `settings-disconnect-sheet`<br>-> `servers` |
| `settings-disconnect-sheet` | sheet | 2 / 4 | `settings` / settings-disconnect | settings-disconnect-sheet-confirm -> `servers` |
| `tools` | screen | 2 / 3 | `chat`<br>`capabilities` / capabilities-tab-tools<br>`command-launcher-sheet` / chat-command-tools | tools-choose-model-empty -> `model-picker-sheet`<br>tools-model-summary -> `model-picker-sheet`<br>tools-header-change -> `model-picker-sheet`<br>tools-callable-row -> `tools-detail-sheet`<br>(embedded) -> `embedded-product-states` |
...
| `guide` | screen | 1 / 1 | `more` / more-setup-guide-tile<br>`about-settings` / about-settings-setup-guide<br>`system` / system-named-route-guide-lib-main-dart-1237-to-guide<br>`servers` / servers-guide<br>`servers` / servers-setup-guide<br>`servers-welcome` / servers-welcome-guide<br>`servers-welcome` / servers-welcome-setup-guide<br>`profile-editor` / profile-editor-test-guide | guide-connection-help -> `connection-help` |
...
| Page | Kind | Depth | Inbound (page / element) | Outbound (element -> page) |
|---|---|---|---|---|
| `embedded-context-menu-region` | overlay | 0 / 2 | `embedded-message-view` / (embedded)<br>`embedded-prompt-error-banner` / (embedded)<br>`files` / (embedded)<br>`global-sessions` / (embedded)<br>`managed-workspaces` / (embedded)<br>`sessions-tab` / (embedded)<br>`terminal` / (embedded)<br>`workspace` / (embedded)<br>`worktrees` / (embedded) | _none_ |
...
| `embedded-product-states` | overlay | 0 / 2 | `active-context` / (embedded)<br>`activity` / (embedded)<br>`chat` / (embedded)<br>`coding-settings` / (embedded)<br>`commands` / (embedded)<br>`files` / (embedded)<br>`global-sessions` / (embedded)<br>`managed-workspaces` / (embedded)<br>`project-health` / (embedded)<br>`projects` / (embedded)<br>`references` / (embedded)<br>`review-workspace` / (embedded)<br>`saved-permissions` / (embedded)<br>`server-settings` / (embedded)<br>`session-context` / (embedded)<br>`session-destination-sheet` / (embedded)<br>`session-relations` / (embedded)<br>`skills` / (embedded)<br>`team-agent` / (embedded)<br>`team-home` / (embedded)<br>`team-run` / (embedded)<br>`terminal` / (embedded)<br>`tools` / (embedded)<br>`voice-notices` / (embedded)<br>`worktrees` / (embedded) | _none_ |
