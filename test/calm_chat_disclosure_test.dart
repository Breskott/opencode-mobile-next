...
Future<void> _pump(WidgetTester tester, {List<MessageWithParts> transcript = const []}) async {
...
void main() {
  testWidgets('idle chat keeps task details available without empty task chrome', (tester) async {
    await _pump(tester);
    ...
    expect(tester.widget<Text>(find.byKey(const Key('chat-title'))).maxLines, 1);
  ...
  testWidgets('tools reveal secondary inputs and prompt operations on demand', (tester) async {
    await _pump(tester);
    await tester.enterText(find.byKey(const Key('chat-composer-field')), 'Keep my draft');
    await tester.pumpAndSettle();
    ...
    expect(tester.widget<TextField>(find.byKey(const Key('chat-composer-field'))).controller!.text, 'Keep my draft');
    expect(tester.takeException(), isNull);
    await tester.pumpWidget(const SizedBox.shrink());
  });
  for (final status in ['running', 'error']) {
    testWidgets('$status tool group keeps progress clear and reveals actionable failures', (tester) async {
      await _pump(tester, transcript: [
        MessageWithParts(
          info: MessageInfo(id: 'tools', sessionID: checkoutSessionID, role: 'assistant'),
          parts: [
            for (final (id, state) in [('read', 'completed'), ('test', status)])
              Part(
                id: id,
                callID: id,
                type: 'tool',
                ...
                toolState: ToolState.fromJson({
                  'status': state,
                  'input': {'command': id == 'read' ? 'cat basket.ts' : 'npm test'},
      ...
      expect(find.byKey(const Key('embedded-tool-row')), status == 'error' ? findsWidgets : findsNothing);
      if (status == 'running') {
